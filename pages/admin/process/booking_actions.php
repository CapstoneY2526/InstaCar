...
// ── UPDATE RESERVATION FROM MODAL (EDIT CREDENTIALS) ─────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['booking_id']) && !isset($_POST['update_booking']) && !isset($_POST['add_manual_booking']))) {
    $booking_id       = intval($_POST['booking_id']);
    $filter           = $_POST['filter'] ?? 'All';
    $source           = $_POST['source'] ?? 'online';
    $fulfillment_type = mysqli_real_escape_string($conn, $_POST['fulfillment_type'] ?? 'pickup');
    $delivery_address = ($fulfillment_type === 'delivery') ? mysqli_real_escape_string($conn, trim($_POST['delivery_address'] ?? '')) : '';
    ...
    $proof_payment_path = $old_booking['proof_payment_path'] ?? '';
    ...
    if (!empty($primary_id_path)) {
        ...
        $customer_folder = "booking_" . $booking_id . '_' . time();
    }
    ...
    $file_keys = [
        ...
        'proof_of_billing' => &$proof_billing_path,
        'proof_of_payment' => &$proof_payment_path
    ];

    foreach ($file_keys as $key => &$path) {
        if (!empty($_FILES[$key]['name']) && $_FILES[$key]['error'] === UPLOAD_ERR_OK) {
            $ext = pathinfo($_FILES[$key]['name'], PATHINFO_EXTENSION);
            $filename = $key . '_' . time() . '.' . $ext;
            if (move_uploaded_file($_FILES[$key]['tmp_name'], $customer_upload_dir . $filename)) {
                $path = $customer_folder . '/' . $filename;
            }
        }
    }
    ...
    if (mysqli_query($conn, $update_sql)) {
        $_SESSION['success'] = "✅ Reservation details updated successfully!";
    } else {
...
if (isset($_GET['id'], $_GET['status'])) {
        ...
        if ($is_extended) {
            ...
            if (mysqli_query($conn, $update_sql)) {
                ...
                $_SESSION['warning'] = "⚠️ EXTENSION / LATE RETURN DETECTED!<br>
                                        ...
                                        ⏰ Extension: +{$extension_hours} hour(s)<br><br>
                                        💰 Original Rate: {$original_rate}<br>
                                        💰 New Rate: {$rate_applied}<br><br>
    ...
    if ($new_status === 'Confirmed') {
        ...
        if ($data) {
            ...
            sendEmail($email, $name, $body, $subject);
            $_SESSION['success'] = "Booking confirmed! Notification sent to $email.";
        }
...
// ── UPDATE BOOKING (ADMIN EDIT FULL) ──────────────────────────────
