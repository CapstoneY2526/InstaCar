...
$current_staff_id = (int)$_SESSION['user_id'];
...
$branch_scope_c  = $staff_branch_id > 0 ? " AND c.branch_id = $staff_branch_id" : " AND 1=0";
...
FROM bookings 
WHERE booking_type = 'manual' $branch_scope";
...
$bookings = [];
$query = "SELECT b.*, 
                 COALESCE(u.name, b.guest_name) AS member_name, 
          ...
          WHERE b.booking_type = 'manual' $branch_scope_b";
...
if ($mine_only) {
    $query .= " AND b.created_by = $current_staff_id";
}
...
// Available cars with busy slots
$available_cars = [];
$carQuery = "
    SELECT c.*, 
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .mine-toggle-wrap input[type="checkbox"] {
        width: 16px;
        height: 16px;
    ...
    body.dark-mode .mine-toggle-wrap label { color: #cbd5e1; }
    ...
    body.dark-mode .stat-icon.bg-brand-yellow {
        ...
        /* ---- Table horizontal scrollbar — dark mode aware ---- */
    .table-responsive {
    ...
    /* Firefox fallback for other scrollbars (prevents the white default) */
    * {
        scrollbar-width: thin;
        scrollbar-color: #cbd5e1 transparent;
    }
            ...
            <div class="p-4">
                ...
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4 header-title-wrapper">
                    <div>
                        ...
                        <p class="text-muted small mb-0">Create and manage walk-in reservations.</p>
                    </div>
                ...
                <div class="d-flex mb-3">
                    <ul class="nav nav-status-pills shadow-sm w-100">
                        <li class="nav-item"><a class="nav-link <?= $filter == 'All' ? 'active' : '' ?>" href="?filter=All<?= $mine_only ? '&mine=1' : '' ?>">All Bookings</a></li>
                        <li class="nav-item"><a class="nav-link <?= $filter == 'Confirmed' ? 'active' : '' ?>" href="?filter=Confirmed<?= $mine_only ? '&mine=1' : '' ?>">Confirmed</a></li>
                        <li class="nav-item"><a class="nav-link <?= $filter == 'Completed' ? 'active' : '' ?>" href="?filter=Completed<?= $mine_only ? '&mine=1' : '' ?>">Completed</a></li>
                        <li class="nav-item"><a class="nav-link <?= $filter == 'Cancelled' ? 'active' : '' ?>" href="?filter=Cancelled<?= $mine_only ? '&mine=1' : '' ?>">Cancelled</a></li>
                        <li class="mine-toggle-wrap">
                            <input type="checkbox" id="mineOnlyToggle" <?= $mine_only ? 'checked' : '' ?>>
                            <label for="mineOnlyToggle"><i class="bi bi-person-check-fill me-1"></i>Mine Only</label>
                        </li>
                    </ul>
                </div>
                ...
                <div class="modal fade" id="manualBookingModal" tabindex="-1" data-bs-backdrop="static" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content glass-card border-0 shadow-lg">
                            ...
                            <div class="modal-body p-4 pt-3">
                                <form action="../admin/process/booking_actions.php" method="POST" enctype="multipart/form-data" id="manualBookingForm">
                    ...
                    <div class="d-block d-md-none mobile-cards-wrapper">
                        ...
                        <?php else: ?>
                            <?php foreach ($bookings as $b): 
                                $is_mine = ($b['created_by'] == $current_staff_id);
                            ?>
                                        ...
                                        <div>
                                            ...
                                            <div class="audit-stack mt-2">
                                                <?php if (!empty($b['staff_name'])): ?>
                                                    <span class="audit-badge <?= $is_mine ? 'is-created-by-staff' : 'is-created-by-other' ?>">
                                                        <i class="bi bi-person-fill"></i>
                                                        By <?= $is_mine ? 'You' : htmlspecialchars($b['staff_name']) ?>
                                                    </span>
                                                <?php else: ?>
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
                                                        ...
                                                        <?php if ($b['status'] == 'Confirmed'): ?>
                                                            <li><a class="dropdown-item text-success" href="../admin/process/booking_actions.php?id=<?= $b['id'] ?>&status=Completed&source=manual&filter=<?= $filter ?>"><i class="bi bi-check-circle me-2"></i>Complete</a></li>
                                                        <?php endif; ?>
                                                        <li><a class="dropdown-item text-danger" href="../admin/process/booking_actions.php?id=<?= $b['id'] ?>&status=Cancelled&source=manual&filter=<?= $filter ?>"><i class="bi bi-x-circle me-2"></i>Cancel</a></li>
                                                    </ul>
                                                </div>
                                            <?php endif; ?>
                                        </div>
    ...
    <div class="modal fade" id="editBookingModal<?= $b['id'] ?>" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content glass-card border-0 shadow-lg">
                ...
                <form method="POST" action="../admin/process/booking_actions.php" enctype="multipart/form-data">
                    <div class="modal-body p-4 pt-3">
                        ...
                        <div class="row g-3">
                            ...
                            <div id="guestInput_<?= $b['id'] ?>" class="col-12 <?= (!empty($b['user_id'])) ? 'd-none' : '' ?>">
                                <div class="row g-3">
                                    <div class="col-12">
                                        ...
                                        <div class="input-group-custom">
                                            <span class="input-icon"><i class="bi bi-person"></i></span>
                                            <input type="text" name="guest_name" id="guestNameInput_<?= $b['id'] ?>" class="form-control custom-input" value="<?= htmlspecialchars($b['guest_name'] ?? '') ?>" <?= (empty($b['user_id'])) ? 'required' : '' ?>>
                                        </div>
                                    </div>
                                    <div class="col-12 col-sm-6">
                                        <label class="form-label text-uppercase fs-xs fw-bold tracking-wider">Email</label>
                                        <div class="input-group-custom">
                                            <span class="input-icon"><i class="bi bi-envelope"></i></span>
                                            <input type="email" name="guest_email" id="guestEmailInput_<?= $b['id'] ?>" class="form-control custom-input" value="<?= htmlspecialchars($b['gmail'] ?? '') ?>">
                                        </div>
                                    </div>
                                    <div class="col-12 col-sm-6">
                                        <label class="form-label text-uppercase fs-xs fw-bold tracking-wider">Phone</label>
                                        <div class="input-group-custom">
                                            <span class="input-icon"><i class="bi bi-telephone"></i></span>
                                            <input type="text" name="guest_phone" id="guestPhoneInput_<?= $b['id'] ?>" class="form-control custom-input" value="<?= htmlspecialchars($b['phone_number'] ?? '') ?>">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-12 col-sm-6">
                                <label class="form-label text-uppercase fs-xs fw-bold tracking-wider text-muted">Primary ID <span class="fw-normal text-lowercase">(leave blank to keep old)</span></label>
                                <div class="file-upload-card">
                            ...
                            <div class="col-12 col-sm-6">
                                <label class="form-label text-uppercase fs-xs fw-bold tracking-wider text-muted">Secondary ID <span class="fw-normal text-lowercase">(optional)</span></label>
                                <div class="file-upload-card">
                            ...
                            <div class="col-12">
                                <label class="form-label text-uppercase fs-xs fw-bold tracking-wider text-muted">Proof of Billing <span class="fw-normal text-lowercase">(leave blank to keep old)</span></label>
                                <div class="file-upload-card">
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group-custom">
                                    ...
                                    <select name="car_id" id="carSelect_<?= $b['id'] ?>" class="form-select custom-select" required>
                                        <?php
                                        $cars_sql = "SELECT id, brand, model, plate_number FROM cars WHERE 1=1 $branch_scope ORDER BY brand ASC";
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group-custom">
                                    <span class="input-icon"><i class="bi bi-chat-left-text"></i></span>
                                    <textarea name="remarks" class="form-control custom-input" rows="2"><?= htmlspecialchars($b['remarks'] ?? '') ?></textarea>
                                </div>
                            </div>
...
function initialize() {
    ...
    const defaultPickup = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`;
    ...
    pickupEl.value = defaultPickup;

    let returnDate = new Date(now);
    returnDate.setHours(returnDate.getHours() + 10);
    returnEl.value = `${returnDate.getFullYear()}-${pad(returnDate.getMonth() + 1)}-${pad(returnDate.getDate())}T${pad(returnDate.getHours())}:${pad(returnDate.getMinutes())}`;

    pickupEl.setAttribute('min', defaultPickup);
    returnEl.setAttribute('min', defaultPickup);
...
function updatePreview() {
    ...
    if (!pickupInput || !returnInput || !previewDiv) return;
...
function filterCarSelectionOptions() {
    ...
    const filterValue = (typeFilterEl?.value || '').toLowerCase();
...
function resetPricingDisplay() {
    const ids = ['displayBasePrice', 'displayDiscount', 'displayDownPayment', 'displayTotal'];
    ids.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerText = id === 'displayDiscount' || id === 'displayDownPayment' ? '-₱0.00' : '₱0.00';
    });
    const tp = document.getElementById('totalPriceInput');
    if (tp) tp.value = "0";
}
...
$(document).ready(function () {
    ...
    function applyLimiterAndFilter() {
        ...
        $('#bookingsTable tbody tr.js-searchable-booking').each(function () {
            if ($(this).text().toLowerCase().includes(queryValue)) {
                desktopCount++;
                $(this).css('display', desktopCount <= limitValue ? '' : 'none');
            } else {
        ...
        $('.mobile-booking-card.js-searchable-booking').each(function () {
            if ($(this).text().toLowerCase().includes(queryValue)) {
                mobileCount++;
                $(this).css('display', mobileCount <= limitValue ? 'block' : 'none');
            } else {
                $(this).css('display', 'none');
            }
        });
    ...
    $('#mineOnlyToggle').on('change', function () {
